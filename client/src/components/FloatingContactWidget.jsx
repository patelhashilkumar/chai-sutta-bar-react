...
const FloatingContactWidget = () => {
    ...
    return (
        <div className="fixed right-0 top-1/2 transform -translate-y-1/2 z-50 flex flex-col items-end filter drop-shadow-xl">
            ...
            {/* Widget Container */}
            <div className={`flex flex-col transition-transform duration-500 ease-in-out ${isOpen ? 'translate-x-0' : 'translate-x-[calc(100%-60px)]'}`}>

                {/* Toggle Button (Black) */}
                <button
                    onClick={toggleWidget}
                    className="w-16 h-16 bg-black text-white flex items-center justify-center rounded-tl-3xl hover:bg-gray-900 transition-all duration-300 hover:w-[70px] relative right-0 hover:right-1 group"
                >
                    <svg
                        ...
                        strokeWidth={2.5}
                        stroke="currentColor"
                        className={`w-6 h-6 transition-transform duration-500 ${isOpen ? 'rotate-0' : 'rotate-180'} group-hover:scale-110`}
                    >
